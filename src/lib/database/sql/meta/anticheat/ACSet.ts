...
export async function logClaim(
  ...
  const formattedDropTimestamp = dayjs(dropTime).format(`YYYY/MM/DD HH:mm:ss`);
...
export async function logBitTransaction(
  giver: GameProfile,
  recipient: GameProfile,
  amount: number,
  guildId: string
...
export async function logItemTransaction(
  giver: GameProfile,
  recipient: GameProfile,
  items: GameItem[],
  guildId: string
...
export async function logMultitrade(
