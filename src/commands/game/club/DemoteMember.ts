...
import { unsetClubModerator } from "../../../lib/database/sql/game/club/ClubSetter";

export default class DemoteMember extends BaseCommand {
  id = `trivium`;
  names = [`demote`];
  description = `Demotes a club moderator to member - you must be the owner to run this command.`;
  ...
  async exec(
    ...
    if (target.id === msg.author.id)
      throw new ClubError.CannotDemoteYourselfError();
    ...
    if (!targetMembership.isMod) throw new ClubError.AlreadyMemberError();

    await unsetClubModerator(targetMembership, club);

    const embed = new MessageEmbed(`Demote`, msg.author).setDescription(
      `:white_check_mark: You demoted **${escapeMarkdown(
        target.tag
      )}** to member in **${escapeMarkdown(club.name)}**.`
    );
