...
export default class OrpheusTotalGiftsReceived extends BaseCommand {
  id = `kano`;
  names = [`ops_tgr`];
  description = `Shows a list of users who have sent gifts to a person.`;
  ...
  async exec(
    ...
    const gifts = await OrpheusService.getGiftsReceived(target);
    ...
    const embed = new MessageEmbed(`Orpheus Total Gifts Received`, msg.author)
      .setTitle(`${target.tag}'s received gifts (${target.id})`)
